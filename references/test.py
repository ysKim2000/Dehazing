...
def SBTE(img, is_only_result=True):
    ...
    # Phase II - White Balance
        ...
        phase = 2
        A = A_WB
        # DIR = 'outputs/results/White Balance Image'
...
if __name__ == "__main__":
    ...
    for(root, directories, files) in os.walk(dir_path):
        for file in files:
            ...
            I = cv2.imread(I_PATH)
            J, J_enhanced, transmission_map, value = SBTE(I, is_only_result=False)
            if(value == 1): DIR = 'outputs/results/Normal'
            if(value == 2): DIR = 'outputs/results/White Balance Image'
            O_PATH = os.path.join(DIR, file)
            result = cv2.hconcat([I, J])
